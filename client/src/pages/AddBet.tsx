...
const formSchema = z.object({
  ...
  amountWagered: z.string().min(1, "Amount wagered is required").refine(val => !isNaN(parseFloat(val)) && parseFloat(val) > 0, "Amount must be a positive number"),
  amountWonLoss: z.string().min(1, "Amount won/loss is required"),
  straightBetOdds: z.string().optional(),
...
export default function AddBet() {
  ...
  const updatePlayerProp = (id: string, field: keyof PlayerProp, value: string) => {
    ...
    if (field === 'sportsbook' && currentBetType === 'parlay' && value) {
      setPlayerProps(prev => prev.map(prop => 
  ...
  const updateWonLossForStraightBet = () => {
    ...
    if (amount > 0 && oddsValue && result === 'Win') {
      let payout = 0;
  ...
  const createBetMutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      const betData = {
        ...
        actualReturn: data.result === 'Win' ? data.amountWonLoss : '0',
        odds: data.straightBetOdds || '',
        status: data.result === 'Win' ? 'won' : data.result === 'Loss' ? 'lost' : 'push',
      ...
      <Card className="w-full">
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-3">
              <Button
                type="button"
                onClick={() => handleBetTypeSelection('straight')}
                className={`flex-1 ${
              ...
              <Button
                type="button"
                onClick={() => handleBetTypeSelection('parlay')}
                className={`flex-1 ${
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          type="number"
                          step="0.01"
                          placeholder={currentBetType === 'straight' ? "Calculated" : "Enter win amount"}
                          {...field}
                          readOnly={currentBetType === 'straight'}
                          className={currentBetType === 'straight' ? "bg-gray-100 text-gray-600" : ""}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
              ...
              <div className="space-y-4">
                <div className="mb-4">
                  <Label className="text-base font-semibold block text-center">
                    {currentBetType === 'straight' ? 'Individual Player/Team Bets' : 'Individual Player/Team Bets'}
                  </Label>
                </div>
                  ...
                  {playerProps.map((prop, index) => (
                    <div key={prop.id} className="p-4 rounded-lg">
                      ...
                      {!advancedFieldsVisible && (
                        <div className={`grid ${currentBetType === 'parlay' ? 'grid-cols-1 md:grid-cols-3' : 'grid-cols-1 md:grid-cols-2'} gap-3 mb-3`}>
                          <div>
                            <Label htmlFor={`${prop.id}-player`} className="sr-only">Player/Team Name</Label>
                            <Input
                              id={`${prop.id}-player`}
                              placeholder={currentBetType === 'straight' ? "Team/Player Name (e.g., Lakers, Mahomes)" : "Player/Team Name (e.g., LeBron James)"}
                              value={prop.player}
                          ...
                          <div>
                            ...
                            <Input
                              id={`${prop.id}-prop`}
                              placeholder={currentBetType === 'straight' ? "Bet Type (e.g., Moneyline, Spread -3.5)" : "Prop Type (e.g., Over 25.5 Pts)"}
                              value={prop.prop}
                      ...
                      {advancedFieldsVisible && (
                        <div className={`grid ${currentBetType === 'parlay' ? 'grid-cols-1 md:grid-cols-5' : 'grid-cols-1 md:grid-cols-4'} gap-3`}>
                          <div>
                      ...
                      {currentBetType === 'parlay' && playerProps.length > 1 && (
                  ...
                  >
                    {createBetMutation.isPending ? (
                      <>
                        ...
                        Adding {currentBetType === 'parlay' ? 'Parlay' : 'Straight Bet'} Entry...
                      </>
                    ) : (
                      <>
                        Add {currentBetType === 'parlay' ? 'Parlay' : 'Straight Bet'} Entry
                      </>
                    )}
                  </Button>
                </div>
