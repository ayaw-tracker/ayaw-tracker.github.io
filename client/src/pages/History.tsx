...
import { BetStatus, ViewMode } from '@shared/types';
import { filterBets } from '@shared/filters';
import { BetRow } from '@/components/ui/BetRow';
...
export default function History({ 
  ...
  // Replace useMemo filtering logic with:
  const filteredBets = useMemo(() => filterBets(bets || [], { viewMode, startDate: startDate || localStartDate, endDate: endDate || localEndDate, searchTerm: debouncedSearchTerm }), [bets, viewMode, startDate, endDate, localStartDate, localEndDate, debouncedSearchTerm]);
  ...
  // In the render, use virtualization if filteredBets.length > 30
  const shouldVirtualize = filteredBets.length > 30;
  const rowHeight = isMobile ? 160 : 64;
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      {!isMobile ? (
        shouldVirtualize ? (
          <List
            height={Math.min(filteredBets.length, 10) * rowHeight + 40}
          ...
          >
            {({ index, style }) => (
              <div style={style} key={filteredBets[index].id}>
                <table style={{ width: '100%' }}>
                  <tbody>
                    <BetRow
                      bet={filteredBets[index]}
                      profit={calculateProfit(filteredBets[index])}
                      onView={handleViewBet}
                      onEdit={handleEditBet}
                      onDelete={() => setDeletingBetId(filteredBets[index].id)}
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredBets.map((bet) => (
                <BetRow
                  ...
                  onView={handleViewBet}
                  onEdit={handleEditBet}
                  onDelete={() => setDeletingBetId(bet.id)}
                  getStatusColor={getStatusColor}
                  getProfitColor={getProfitColor}
                  formatCurrency={formatCurrency}
                />
